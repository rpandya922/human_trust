...
csv_headers = ['user id', 'robot', 'observe_reward']
csv_dataset = []
...
for user in user_data.keys():
    ...
    for key in observe_keys:
        ...
        if robot == 'greedy':
            ...
            g2.append(total_reward)
        elif robot == 'optimal':
            o.append(total_reward)
        elif robot == 'random':
            r.append(total_reward)
        csv_dataset.append([user, robot, total_reward])

## writing data to csv
with open('../data/mturk_observe_dataset.csv', 'wb') as f:
